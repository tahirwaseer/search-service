...
const search = (query: string, geo_location: string): SearchResult => {
  const point: Array<string> = geo_location.split(',')
  ...
  const searchResults: Array<ServiceResponse> = findMatch(query, indexedNames, options).map(([name, score, index]: FuzzBallResult): ServiceResponse  => {
    ...
    let distanceKm: number = StraightLineDistance(lat, lng, originalDocument.position.lat, originalDocument.position.lng)
